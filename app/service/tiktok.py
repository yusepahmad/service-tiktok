...
class Tiktok():
    ...
    def followers(self, username, limit):
        ...
        if script_tag:
            ...
            iteration_count = 0
            while True:
                response = requests.get('https://www.tiktok.com/api/user/list/', params=param, cookies=cookies,
                                        headers=header)
                json_data = response.json()
                ...
                if len(userlist) != 0 or max_cursor == -1 or min_cursor == -1 or iteration_count >= limit:
                    break
                param['maxCursor'] = str(max_cursor)
                param['minCursor'] = str(min_cursor)
                iteration_count += 1
